...
# Module-level memory for de-duping consecutive states
_LAST_LABEL_KEY = None  # e.g., "READING", "REJECTING", "NOTE_READ:ch=2", "UNKNOWN:0xED"
...
def _emit_on_change(label_key: str, line: str):
    """Print only when the state (label_key) changes."""
    global _LAST_LABEL_KEY
    if _LAST_LABEL_KEY == label_key:
        return
    _LAST_LABEL_KEY = label_key
    print(line)


def _print_event(ev, validator: NV9Validator):
    """
    ...
    if name == "READING":
        _emit_on_change("READING", "[EVENT] READING")
        return

    if name == "REJECTING":  # 0xED mapped in nv9_core
        _emit_on_change("REJECTING", "[EVENT] REJECTING")
        return

    if name == "STACKING":
        _emit_on_change("STACKING", "[EVENT] STACKING")
        return

    if name == "STACKED":
        _emit_on_change("STACKED", "[EVENT] STACKED")
        return
    ...
    if name == "NOTE_READ":
        ...
        if ch is not None and val is not None:
            _emit_on_change(key, f"[EVENT] NOTE_READ (ch {ch}, EUR {val})")
        elif ch is not None:
            _emit_on_change(key, f"[EVENT] NOTE_READ (ch {ch})")
        else:
            _emit_on_change(key, "[EVENT] NOTE_READ")
        return

    if name in ("DISABLED", "SLAVE_RESET"):
        _emit_on_change(name, f"[EVENT] {name}")
        return
    ...
    if name.startswith("UNKNOWN("):
        ...
        _emit_on_change(key, f"[EVENT] {name}")
    else:
        _emit_on_change(name, f"[EVENT] {name}")
